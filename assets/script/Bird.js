...
cc.Class({
    ...
    onCollisionEnter(other, self) {
        ...
        if (coinPosition.length === 0) { this.createCoin(coinPosition, angle) } else {
            let less = false;
            for (let i of coinPosition){
                let num = angle - i;
                if(num>-20&&num<20) less = true;
            }
            if(!less){this.createCoin(coinPosition, angle)}else{cc.director.pause();}
        }
    },
